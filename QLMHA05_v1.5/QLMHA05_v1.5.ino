/*
  ...
  This is the version currently we are using for application
changes :-
1. two different topic using for communication with aws
2. device id will be taken manualy from 192.168.4.1/id?deviceId=COCA-CH5-A0
 ...
 while ((WiFi.status() != WL_CONNECTED)  && retry < 15)
  {
    ...
    digitalWrite(internetStatusLed,HIGH);
      ...
      server.on("/id", []() {
          ...
          EEPROM.writeString(400,deviceIDString);
          EEPROM.commit();
...
void keepWiFiAlive(void * pvParameter){
   while(1)
   {
        if(WiFi.status() == WL_CONNECTED)
          {
            vTaskDelay(900000 / portTICK_PERIOD_MS);    // every 15 minutes
            sendResponse();
            continue;
          }
