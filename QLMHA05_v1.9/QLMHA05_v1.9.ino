/*
 ...
 8. toggle the operating mode on Single press
...
dimmer.setState(OFF);
 ...
 if (!EEPROM.begin(EEPROM_SIZE)){      //Initialasing EEPROM
        delay(1000);
 }
...
/* Topic arrangment */
 ...
 modeId = EEPROM.read(modeAddress);
  Serial.println("mode = " + String(modeId));
  EEPROM.write(modeAddress, modeId!=0 ? 0 : 1);
  EEPROM.commit();

  if(modeId != 0)             /********local server setup*******/
  { 
        manual_mode();
        setupLocalMode();
  }else                       /****internet mode setup******/
  {   
    setupInternetMode();
...
/***************intetrnet mode setup**************/
void setupInternetMode()
{
        for (int i = 0; i < 32; ++i)
        {
